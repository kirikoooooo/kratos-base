...
func TestUpdateContextUsageRecordsCompressCount(t *testing.T) {
	...
	taskID := "ctx-task-1"
	trace.StartTask(taskID, biz.TaskAgentCoder, biz.TaskStatusRunning)

	usage := biz.NewContextUsageSnapshot(biz.ContextCompressStats{
		EstimatedChars: 210000,
		Threshold:      200000,
		NeedsCompress:  true,
	})
	compress := &biz.ContextCompressResult{
	...
	trace.AppendEvent(biz.DelegationEvent{
		...
		Summary: biz.FormatContextCompressEventSummary(*compress),
	})
