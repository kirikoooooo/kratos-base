...
func (s *fileAgentMemoryStore) LoadUser(_ context.Context, userID string) (*UserAgentMemory, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	if memory == nil {
		memory = &UserAgentMemory{UserID: userID}
	}
	memory.UserID = userID
	return memory, nil
}

func (s *fileAgentMemoryStore) SaveUser(_ context.Context, memory *UserAgentMemory) error {
	if memory == nil {
...
func (s *fileAgentMemoryStore) LoadSession(_ context.Context, sessionID string) (*SessionAgentMemory, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	if memory == nil {
		memory = &SessionAgentMemory{SessionID: sessionID}
	}
	memory.SessionID = sessionID
	return memory, nil
}

func (s *fileAgentMemoryStore) SaveSession(_ context.Context, memory *SessionAgentMemory) error {
	if memory == nil {
...
func (s *fileAgentMemoryStore) AppendSessionError(_ context.Context, record SessionErrorRecord) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *fileAgentMemoryStore) ListSessionErrors(_ context.Context, sessionID string, limit int) ([]SessionErrorRecord, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	var records []SessionErrorRecord
...
func (s *fileAgentMemoryStore) LoadConversation(_ context.Context, sessionID string) (*SessionConversation, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	if memory == nil {
		memory = &SessionConversation{SessionID: sessionID}
	}
	memory.SessionID = sessionID
	return memory, nil
}

func (s *fileAgentMemoryStore) SaveConversation(_ context.Context, memory *SessionConversation) error {
	if memory == nil {
...
func (s *fileAgentMemoryStore) readConversationFile(path string) (*SessionConversation, error) {
	raw, err := os.ReadFile(path)
	if err != nil {
		if os.IsNotExist(err) {
			return nil, nil
	...
	var memory SessionConversation
...
func (s *fileAgentMemoryStore) readUserFile(path string) (*UserAgentMemory, error) {
	raw, err := os.ReadFile(path)
	if err != nil {
		if os.IsNotExist(err) {
			return nil, nil
...
func (s *fileAgentMemoryStore) readSessionFile(path string) (*SessionAgentMemory, error) {
	raw, err := os.ReadFile(path)
	if err != nil {
		if os.IsNotExist(err) {
			return nil, nil
