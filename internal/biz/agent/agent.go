...
import (
	...
	agentruntime "kratos-demo/internal/biz/agent_runtime"
...
//
// Memory 通过 Runtime 间接访问——agentRuntime 内部已持有 AgentMemory，
...
func NewAgent(runtime agentruntime.AgentRuntime, tools biztool.ToolRepository) *Agent {
	return &Agent{
		Runtime: runtime,
		Tools:   tools,
	}
...
func (a *Agent) ExecuteTask(ctx context.Context, cmd *taskv1.TaskCommand) (*taskv1.TaskResult, error) {
	if a == nil || a.Runtime == nil {
		return nil, agentruntime.ErrAgentRuntimeUnavailable
	}
	if cmd == nil {
...
func (a *Agent) Execute(ctx context.Context, kind agentruntime.AgentKind, prompt string) (*taskv1.TaskResult, error) {
	if a == nil || a.Runtime == nil {
		return nil, agentruntime.ErrAgentRuntimeUnavailable
	}
	if !a.Runtime.Supports(kind) {
		return nil, agentruntime.ErrAgentNotSupported
	}
	return a.Runtime.Execute(ctx, kind, prompt)
}

// VerifyDelegation verifies a delegated task result.
func (a *Agent) VerifyDelegation(ctx context.Context, taskID string, kind agentruntime.AgentKind, prompt string) (*taskv1.TaskResult, error) {
	if a == nil || a.Runtime == nil {
		return nil, agentruntime.ErrAgentRuntimeUnavailable
	}
	verifier, ok := a.Runtime.(agentruntime.DelegationVerifier)
	if !ok {
		return nil, agentruntime.ErrDelegationNotSupported
	}
